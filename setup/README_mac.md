# macOS/Linux Setup & Onboarding Guide

This guide helps team members set up the WrenAI + DuckDB + FastAPI dashboard workspace on macOS and Linux systems.

## Prerequisites

Before running the setup, ensure your system has the following:
1. **Python 3.11 or later**: Verify via `python3 --version`. If not installed, you can install it using Homebrew (`brew install python@3.12`) or download it from [python.org](https://www.python.org/downloads/).
2. **Git**: Verify via `git --version`.
...
5. Copy the generated key and save it securely. You will need to export this key as an environment variable:
   ```bash
...
We have provided a bash script [setup_mac.sh](file:///Users/harshit/Desktop/SN/setup/setup_mac.sh) to fully automate the installation and configuration of the environment.

To run it, navigate to the project root directory and execute:
```bash
...
python3 setup/update_profile.py
```
